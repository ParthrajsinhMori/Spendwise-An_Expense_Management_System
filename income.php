...
<body>
    <div class="container-custom">
        <button type="button" class="btn btn-custom" id="btn">Add Income</button>
...
<script>
    ...
    // xhr.open("GET","getincome.php?email="+email,true);
    ...
    $.ajax({
        ...
        success:function(response){
            ...
            for(i=0;i<response.length;i++){
                output+="<tr style='background-color: white; box-shadow: 0 4px 8px rgba(0, 0, 0, 0.1); transition: transform 0.2s ease;'>"+"<td style='padding: 15px;'>"+response[i].date+"</td>"+"<td style='padding: 15px;'>"+response[i].category+"</td>"+"<td style='padding: 15px;'>"+response[i].amount+"</td>"+"<td style='padding: 15px;'>"+response[i].additional_info+"</td>"+"<td><button id='btnEdit' class='btn btn-warning' data-sid="+response[i].srno+">Edit</button><button id='btnDelete' class='btn btn-danger' data-sid="+response[i].srno+">Delete</button></td>"+"</tr>";
            }
            $('#output').html(output);
        }
    });
